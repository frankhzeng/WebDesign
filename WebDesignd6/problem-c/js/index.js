...
let button1 = document.querySelector("#add-task");
...
function createTask(obj) {
  ...
  out.textContent = obj.description;
  if (obj.complete) {
    out.classList.add("font-strike");
  }
...
console.log(createTask(state.taskList[0]));
...
function renderTaskList() {
  ...
  for (let i = 0; i < state.taskList.length; i++) {
    parent.appendChild(createTask(state.taskList[i]));
  }
  renderInput();
}
...
function addNewTask() {
  let newTask = {id: state.taskList[state.taskList.length - 1].id + 1, description: state.inputtedText, completed: false};
