...
let arr = huskyGames2016.map(x => extractOpponent(x));


//Define a function `huskiesLost()` that takes in a "game" object and returns
//whether or not UW lost.
function huskiesLost(obj) {
  if (obj.home == "UW") {
    return obj.home_score < obj.opponent_score;
  }
  if (obj.opponent == "UW") {
    return obj.home_score > obj.opponent_score;
  }
...
/*let gameArray = function(obj){
    return obj["home"]+" at "+obj["opponent"]+", "+obj["home_score"]+" to "+obj["opponent+score"];
    }*/

arrLost.forEach(function(obj) {
  console.log( obj.opponent + " at " + obj.home + ", " + obj.home_score + " to " + obj.opponent_score + ", " + (obj.home_score > obj.opponent_score ? obj.home : obj.opponent) + " won." );
})
...
let fumbles = huskyGames2016.filter(x => x.fumbles > 0);

// for (let i = 0; i < fumbles.length; i++) {
//   console.log(fumbles[i]);
// }
...
function mostYardsPassing(obj1, obj2) {
  if (!obj1.passing_yards) {
    return obj2;
  }
  return obj1.passing_yards > obj2.passing_yards ? obj1 : obj2;
}
